...
function removeScrollLockClasses(): void {
  if (
    document.documentElement.classList.contains("simple-shield-disabled") ||
    document.documentElement.classList.contains("simple-shield-cookie-banners-disabled")
  ) return;

  if (!document.querySelector(COOKIE_BANNER_SELECTORS.join(","))) return;
