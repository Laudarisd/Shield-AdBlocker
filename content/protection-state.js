...
function isCurrentSiteAllowlisted(value) {
    if (!Array.isArray(value))
        return false;
    const hostname = location.hostname.toLowerCase().replace(/^www\./, "");
    return value.some((site) => typeof site === "string" &&
        (hostname === site || hostname.endsWith(`.${site}`)));
}
function applyPageProtectionClasses(stored) {
    const root = document.documentElement;
    root.classList.toggle(CONTENT_MASTER_DISABLED_CLASS, stored.enabled === false || isCurrentSiteAllowlisted(stored.allowlistedSites));
...
async function refreshPageProtectionClasses() {
...
document.documentElement.classList.add(CONTENT_MASTER_DISABLED_CLASS, CONTENT_AD_ELEMENTS_DISABLED_CLASS, CONTENT_COOKIE_BANNERS_DISABLED_CLASS, CONTENT_YOUTUBE_DISABLED_CLASS);
void refreshPageProtectionClasses();
chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === "local" &&
        ["enabled", "hideAdElements", "hideCookieBanners", "skipYouTubeAds", "allowlistedSites"].some((key) => changes[key] !== undefined)) {
        void refreshPageProtectionClasses();
    }
});
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.kind !== "GET_PAGE_SITE")
        return false;
